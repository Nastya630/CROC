...
public final class ChessPositionParser {
    ...
    public static ChessPosition parse(final String position) throws IllegalPositionException{
       ChessPositionImpl pos = new ChessPositionImpl(position);
       if (pos.x()<0 || pos.x()>7 || pos.y()<0 || pos.y()>7 || position.length()>2){
           throw new IllegalPositionException(); //Если позиция не удовлетворяет нужным параметрам, то выбрасываем IllegalPositionException
       }
       return pos;
    }
}
